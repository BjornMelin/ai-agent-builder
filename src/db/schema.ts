...
export const projectsTable = pgTable(
  ...
  (t) => [
    ...
    uniqueIndex("projects_owner_user_id_slug_unique").on(t.ownerUserId, t.slug),
...
export const chatThreadsTable = pgTable(
  "chat_threads",
  {
    ...
    lastActivityAt: timestamp("last_activity_at", { withTimezone: true })
      ...
      .defaultNow(),
    /**
     * Selected agent mode for this thread (tool allowlist + system prompt).
     */
    mode: varchar("mode", { length: 64 }).notNull().default("chat-assistant"),
    projectId: uuid("project_id")
      .notNull()
      .references(() => projectsTable.id, { onDelete: "cascade" }),

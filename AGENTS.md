...
bun run build                  # Build for production with native Next.js/Workflow integration
bun run format                 # Format code with Biome
bun run lint                   # Run Biome and ESLint
bun run knip                   # Find unused files and dependencies
bun run typecheck              # Run TypeScript compiler checks
bun run test                   # Run tests with Vitest
bun run ci                     # Run lint/knip/typecheck/test/build
...
- Keep provider construction and model resolution in `src/lib/ai/gateway.server.ts`; workflows must use `getChatModelById` instead of defining parallel gateway adapters.
...
- Project chat uses AI SDK 7 `WorkflowAgent` from `@ai-sdk/workflow`; do not add a parallel durable-agent wrapper.
- Keep the default workflow stream typed as `UIMessageChunk`. Write `ModelCallStreamPart` values to private turn namespaces and convert them in `src/workflows/chat/steps/assistant-turn-stream.step.ts` so reconnect cursors match client-visible chunks.
- Build a fresh chat toolset for every assistant turn. Pass immutable scope through `toolsContext` and validate scoped tools with `contextSchema`.
